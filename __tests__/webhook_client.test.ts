...
describe("Webhook Client tests", () => {
  ...
  const startWebhook = ({ token, expressApp, port, path, appSecret }: WebhookClientArgs) => {
  ...
  it("passes a verify callback to json() when appSecret is provided", () => {
    startWebhook({ token: "", appSecret: "my-secret" });
    expect(json).toHaveBeenCalledWith({ verify: expect.any(Function) });
  });

  it("mounts verifyHubSignature before the event handler when appSecret is provided", () => {
    class RouterSpy extends WebhookClient {
      getRouter() {
        this.initWebhook(events);
        return this.router;
      }
    }
    const router = new RouterSpy({ token: "", appSecret: "my-secret" }).getRouter();
    // POST should be called with 3 handlers: path, verifier fn, and event controller fn
  ...
  it("webhook handler fires the events", () => {
    ...
    // value is MessagingWebhookValue for this test body (field: "messages")
    const msgValue = body.entry[0].changes[0]
      .value as import("../src/types/webhooks").MessagingWebhookValue;
    const contact = msgValue.contacts[0];
    const metadata = msgValue.metadata;
